...
// Grow-From-Root helper (greedy warm-start)
void gfr_grow(Tree& t,
              const arma::mat& X,
              ...
              int max_depth = 5)
{
    ...
    t.node_vector[0].is_terminal = t.node_vector[0].in_use = true;
    ...
    for(int depth = 0; depth < max_depth; ++depth) {
        ...
        for(size_t i = 0; i < t.node_vector.size(); ++i) {
            if(t.node_vector[i].is_terminal && t.node_vector[i].in_use) {
                Tree tmp = t;
                tmp.grow(X, X_test, p, min_nodesize);
                if(tmp.node_vector.size() > t.node_vector.size()) {
                    t = std::move(tmp);
                    any_split = true;
                }
            }
...
// Warm-start via Grow-From-Root passes
if(num_gfr > 0){
  Rcpp::Rcout << "Running GFR warm-start (" << num_gfr << " passes)..." << std::endl;
  for(int pass=0; pass<num_gfr; ++pass){
    for(int t=0;t<n_tree;++t)
      gfr_grow(bart_forest.tree_vector[t], X_con, X_con_test, p, min_nodesize);
    for(int t=0;t<n_tree_tau;++t)
      gfr_grow(tau_forest.tree_vector[t], X_mod, X_mod_test, p_tau, min_nodesize);
  }
  for(int t=0;t<n_tree;++t){
    tree_preds.slice(t)      = bart_forest.tree_vector[t].get_predictions(outcomes);
    tree_preds_test.slice(t) = bart_forest.tree_vector[t].get_test_predictions(outcomes);
  }
  for(int t=0;t<n_tree_tau;++t){
    tree_preds_tau.slice(t)      = tau_forest.tree_vector[t].get_predictions(outcomes);
    tree_preds_tau_test.slice(t) = tau_forest.tree_vector[t].get_test_predictions(outcomes);
  }
  Rcpp::Rcout << "GFR warm-start done." << std::endl;
}
